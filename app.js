...
//list of all states in the state table
...
//list of state based on state_id
app.get('/states/:stateId/', async (request, response) => {
  ...
  response.send(objectSnakeToCamel(newState))
})

//create new district in district table
app.post('/districts/', async (request, response) => {
  ...
  const newDistricts = `
    ...
    `
  await db.run(newDistricts)
  response.send('District Successfully Added')
})

//returns district based on district_id
...
//update details of district based on district_id
...
//returns statistics of total cases
app.get('/states/:stateId/stats/', async (request, response) => {
  const {stateId} = request.params
  const getReportStatsQuery = `
      ...
      WHERE state_id = ${stateId};
    `
  const stats = await db.get(getReportStatsQuery)
  console.log(stats)
  response.send({
    totolCases: stats['SUM(cases)'],
